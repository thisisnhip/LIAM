    ...
    public partial class frmKhachHang : Form
    {
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(sCon))
            {
                ...
                reader.Close();
                // Kiểm tra trùng SDT với khách hàng khác
                SqlCommand checkSDT = new SqlCommand(
                    "SELECT COUNT(*) FROM KhachHang WHERE SDT_KH = @sdt AND MaKH <> @maKH", con);
                checkSDT.Parameters.AddWithValue("@sdt", sdt);
                checkSDT.Parameters.AddWithValue("@maKH", maKH);

                int sdtCount = (int)checkSDT.ExecuteScalar();
                if (sdtCount > 0)
                {
                    MessageBox.Show("Số điện thoại này đã tồn tại ở khách hàng khác.");
                    return;
                }
        ...
        private void btnLocKHVIP_Click(object sender, EventArgs e)
        {
            int nam;
            if (!int.TryParse(txtNam.Text.Trim(), out nam))
            {
                MessageBox.Show("Vui lòng nhập năm hợp lệ (ví dụ: 2024)");
                return;
            }

            using (SqlConnection con = new SqlConnection(sCon))
            {
                SqlCommand cmd = new SqlCommand("sp_CapNhatKhachHangVIP", con);
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                try
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        // Đổi tiêu đề cột nếu cần
                        dgvKhachHang.Columns["MaKH"].HeaderText = "Mã KH";
                        dgvKhachHang.Columns["Khách hàng VIP"].HeaderText = "Tên KH";
                        dgvKhachHang.Columns["SDT_KH"].HeaderText = "SĐT";
                        dgvKhachHang.Columns["Tổng thanh toán"].HeaderText = "Tổng Chi Tiêu";
                    }
                    else
                    {
                        MessageBox.Show("Không có khách hàng VIP trong năm " + nam);
                    }
                }
                catch (Exception ex)
                {
                    MessageBox.Show("Lỗi khi lọc khách VIP: " + ex.Message);
                }
            }
        }
    }
